...
type ProcessCursorCleanup = () => void;

function showTerminalCursor(): void {
  try {
    process.stdout.write(SHOW_TERMINAL_CURSOR);
  } catch {
    // Best-effort cleanup for exit/crash paths.
  }
}

let processCursorCleanupRefCount = 0;
let removeProcessCursorCleanup: (() => void) | null = null;

function installProcessCursorCleanup(): ProcessCursorCleanup {
  processCursorCleanupRefCount += 1;
  if (!removeProcessCursorCleanup) {
    const onExit = () => showTerminalCursor();
    const onSigint = () => {
      showTerminalCursor();
      process.exit(130);
    };
  ...
  return () => {
    if (disposed) return;
    disposed = true;
    processCursorCleanupRefCount = Math.max(0, processCursorCleanupRefCount - 1);
    if (processCursorCleanupRefCount === 0) {
      removeProcessCursorCleanup?.();
      removeProcessCursorCleanup = null;
    }
  };
}
...
export default function (pi: ExtensionAPI) {
  ...
  const processCursorCleanup = installProcessCursorCleanup();
